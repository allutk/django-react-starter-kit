...
export default function AuthInputField({
  ...
  return (
    <div>
      <div className={error ? "relative h-14 w-full" : "relative h-14 w-full mb-6"}>
        <input className={
        ...
        <i className={"absolute material-symbols-rounded top-1/2 left-4 " +
                      "-translate-y-1/2 pointer-events-none text-black"}>
          {icon}
        </i>

        {type === "password" && (
          <i className={"absolute material-symbols-rounded top-1/2 right-4 cursor-pointer " +
      ...
      {error && <div className="text-sm mt-1 mb-3 text-error">
        {error.message}
      </div>}
    </div>
  );
